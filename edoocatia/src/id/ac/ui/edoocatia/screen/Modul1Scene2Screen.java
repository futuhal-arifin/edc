...
public class Modul1Scene2Screen extends DialogScreen {
	...
	public Modul1Scene2Screen(Edoocatia app) {
		...
		background = new Texture(
				Gdx.files
						.internal("data/images/modul-1/background/background.jpg"));

		if (app.getEdocatiaData().isModul1Scene3Done()
				&& app.getEdocatiaData().isModul1Scene5Done()) {
		...
		this.pesawatBody = new Texture(
				Gdx.files.internal("data/images/modul-1/pesawat/badan.png"));
		this.pesawatParts[this.SPION] = new Texture(
				Gdx.files.internal("data/images/modul-1/pesawat/spion.png"));
		this.pesawatParts[this.SAYAP] = new Texture(
				Gdx.files.internal("data/images/modul-1/pesawat/sayap.png"));

		this.pesawatPartsSelected[this.SPION] = new Texture(
				Gdx.files
						.internal("data/images/modul-1/pesawat/spion_gelap.png"));
		this.pesawatPartsSelected[this.SAYAP] = new Texture(
				Gdx.files
						.internal("data/images/modul-1/pesawat/sayap_gelap.png"));
		...
		if (!this.status[this.SPION_IS_DONE]) {
			float spionLeftBound = (width - this.pesawatParts[this.SPION]
					.getWidth()) / 2;
			float spionRightBound = (width + this.pesawatParts[this.SPION]
					.getWidth()) / 2;
			...
			spionBounds = new Rectangle[] {
					new Rectangle(spionLeftBound, this.spionYPosition
							+ (spionHeight / 2), spionRect1Width,
							spionHeight / 2),
					...
					new Rectangle(spionRightBound - spionRect1Width,
							this.spionYPosition + (spionHeight / 2),
							spionRect1Width, spionHeight / 2),
					new Rectangle(spionRightBound - spionRect2Width
							- spionRect1Width, this.spionYPosition,
							spionRect2Width, spionHeight), };
		}

		if (!this.status[this.SAYAP_IS_DONE]) {
			float sayapLeftBound = (VIRTUAL_WIDTH - this.pesawatParts[this.SAYAP]
					.getWidth()) / 2;
			float sayapRightBound = (VIRTUAL_WIDTH + this.pesawatParts[this.SAYAP]
					.getWidth()) / 2;
			...
			sayapBounds = new Rectangle[] {
					new Rectangle(sayapLeftBound, this.sayapYPosition
							+ sayapHeight / 2, sayapRect1Width,
							(sayapHeight) / 2),
					new Rectangle(sayapRect1Width + sayapLeftBound,
							(sayapHeight / 2), sayapRect2Width, 230),
					new Rectangle(-5 + sayapRect1Width + sayapRect2Width
							+ sayapLeftBound, (sayapHeight / 2) - 95,
							sayapRect3Width, 235),
					new Rectangle(-5 + sayapRect1Width + sayapRect2Width
							+ sayapRect3Width + sayapLeftBound,
							this.sayapYPosition, sayapRect4Width, 200),

					new Rectangle(-sayapRect1Width + sayapRightBound,
							this.sayapYPosition + sayapHeight / 2,
							sayapRect1Width, sayapHeight / 2),
					new Rectangle(-(sayapRect1Width + sayapRect2Width)
							+ sayapRightBound, (sayapHeight / 2),
							sayapRect2Width, 230),
					new Rectangle(
							-(-5 + sayapRect1Width + sayapRect2Width + sayapRect3Width)
									+ sayapRightBound, (sayapHeight / 2) - 95,
							sayapRect3Width, 235),
					new Rectangle(-(-5 + sayapRect1Width + sayapRect2Width
							+ sayapRect3Width + sayapRect3Width)
							+ sayapRightBound, this.sayapYPosition,
							sayapRect4Width, 200), };
		}
	...
	public void render(float delta) {
		...
		batcher.draw(this.pesawatBody,
				(VIRTUAL_WIDTH - this.pesawatBody.getWidth()) / 2, 100);

		if (this.isShowDialog() || this.partIsSelected(this.SAYAP)
				|| this.status[this.SAYAP_IS_DONE]) {
			batcher.draw(
					this.pesawatParts[this.SAYAP],
					(VIRTUAL_WIDTH - this.pesawatParts[this.SAYAP].getWidth()) / 2,
					100);
		} else {
			batcher.draw(
					this.pesawatPartsSelected[this.SAYAP],
					(VIRTUAL_WIDTH - this.pesawatParts[this.SAYAP].getWidth()) / 2,
					100);
		}

		if (this.isShowDialog() || this.partIsSelected(this.SPION)
				|| this.status[this.SPION_IS_DONE]) {
			batcher.draw(
					this.pesawatParts[this.SPION],
					(VIRTUAL_WIDTH - this.pesawatParts[this.SPION].getWidth()) / 2,
					300);
		} else {
			batcher.draw(
					this.pesawatPartsSelected[this.SPION],
					(VIRTUAL_WIDTH - this.pesawatParts[this.SPION].getWidth()) / 2,
					300);
		}
		...
		if (this.status[this.SAYAP_IS_DONE] && this.status[this.SPION_IS_DONE]) {
			...
			batcher.draw(currentFrame,
					VIRTUAL_WIDTH - (currentFrame.getRegionWidth()), 0);
		}
	...
	private void setPlayerAnimation() {
		playerTexture = this.getApp().getEdocatiaData().getPlayer()
				.getKarakterWinTexture();
	...
	private void setProfessorInfo() {
		...
		this.setDialogBackgroundPosition(
				(VIRTUAL_WIDTH - this.dialogBackground.getWidth()) / 2, 0);
